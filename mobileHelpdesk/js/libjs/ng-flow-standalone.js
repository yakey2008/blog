...
(function(window, document, undefined) {'use strict';
  ...
  Flow.prototype = {
    ...
    assignBrowse: function (domNodes, isDirectory, singleFile, attributes) {
      ...
      each(domNodes, function (domNode) {
        ...
        input.addEventListener('change', function (e) {
            //alert("position:4|" + e.target.value);
            //e.target.value = encodeURI(e.target.value); 
            //e.target.value = decodeURI(e.target.value); 
            //alert("position:5|" + e.target.value);
            if (e.target.value) {
                $.addFiles(e.target.files, e);
                console.log(fileNativeData);
            e.target.value = '';
       	  }
        }, false);
      }, this);
    ...
    addFiles: function (fileList, event) {
      ...
      each(fileList, function (file) {
        // https://github.com/flowjs/flow.js/issues/55
          if ((!ie10plus || ie10plus && file.size > 0 && file.size < 10485760) && !(file.size % 4096 === 0 && (file.name === '.' || file.fileName === '.')) &&
          (this.opts.allowDuplicateUploads || !this.getFromUniqueIdentifier(this.generateUniqueIdentifier(file)))) {
          var f = new FlowFile(this, file);
          if (this.fire('fileAdded', f, event)) {

              //alert("position:1-1|type:" + f.type + "|name:" + f.name );

              //f.name = decodeURI(f.name);

              //alert("position:1-2|type:" + f.type + "|name:" + f.name );

              if (f.type == "" || f.type == "undefined") {
                  var nativeData = getNativeDataByName(fileNativeData, f.name);
                  var nameIndex = nativeData.indexOf("$");
                  var typeIndex = nativeData.indexOf("@");
                  f.nativeName = nativeData.substring(nameIndex + 1, typeIndex);
                  f.typeIndex = nativeData.substring(typeIndex + 1, nativeData.length);
                  f.IsDefaultNative = false;
                  //alert("position:1-3|nativeName:" + f.nativeName + "|typeIndex:" + f.typeIndex);
              }
              files.push(f);
    ...
    getSize: function () {
      ...
      /**
        * zjx added  ���Ӱ�λ�ķ���:KB MB
        * @function
        * @returns {string}
        */
    getSizeWithUnit: function () {
        var totalSize = 0;
        each(this.files, function (file) {
            totalSize += file.size;
        });
        
        return showSizeWithUnit(totalSize);
    },
  ...
  function FlowFile(flowObj, file) {
    ...
    this.type = file.type;
       ...
       * @type {string}
       */
    this.sizeWithUnit = showSizeWithUnit(file.size);
       ...
       * @type {boolean}
       */
    this.sizeExceeded = file.size > this.flowObj.opts.chunkSize;


      /**
       * zjx added ������Guid
       */
    this.fileGuid = newGuid();
    ...
    this.nativeName = file.name;
       ...
       * zjx added ��ȡ�ļ���׺��
       */
    this.nativeType = file.type;


      /**
       * zjx added �Ƿ�Ĭ��ȡֵԭ�����
       */
    this.isDefaultNative = true;
  ...
  FlowFile.prototype = {
     ...
     * @returns {string}
     */
    getNativeExtension: function () {
    	if(this.IsDefaultNative==false)
    	{
            return this.nativeName.substr((~-this.nativeName.lastIndexOf(".") >>> 0) + 2).toLowerCase();
    	}
    	else
    	{
            return this.name.substr((~-this.name.lastIndexOf(".") >>> 0) + 2).toLowerCase();
  ...
  FlowChunk.prototype = {
    ...
    getParams: function () {
      return {
        ...
        flowFileGuid: this.fileObj.fileGuid,
        flowIsDefaultNative: this.fileObj.isDefaultNative,
        flowFileNativeData: fileNativeData,
        flowFileNativeName: this.fileObj.nativeName,
        flowFileNativeType: this.fileObj.nativeType,
...
angular.module('flow.stylebg', ['flow.init'])
.directive('flowStylebg', [function() {
  return {
    'scope': false,
    'require': '^flowInit',
    'link': function(scope, element, attrs) {
      var file = attrs.flowStylebg;
      scope.$watch(file, function (file) {
        if (!file) {
          return ;
        }
        ...
        fileReader.onload = function (event) {
          scope.$apply(function () {
            attrs.$set('style', 'background:url('+event.target.result+') no-repeat;background-size: 100%;');
          });
        };
      });
    }
...
angular.module('flow', ['flow.provider', 'flow.init', 'flow.events', 'flow.btn',
  'flow.drop', 'flow.transfers', 'flow.img','flow.stylebg' ,'flow.dragEvents']);


/*
  ��ʾ�ļ���С����MB��KBΪ��λ������2λС��
*/
function showSizeWithUnit(size) {

    var unitMB = 1024 * 1024;
    var unitKB = 1024;
    if (size > unitMB) {
        //return (size / unitMB).toFixed(2) + "MB";
        return reserveTwoDecimal(size / unitMB) + "MB";
    }
    else {
        return reserveTwoDecimal(size / unitKB) + "KB";
    }
}

//������λС��  
//���ܣ����������������룬ȡС����2λ 
function reserveTwoDecimal(x) {
    var f = parseFloat(x);
    if (isNaN(f)) {
        return;
    }
...
//��ȡ�ϴ��������ļ���
//var fileNativeData = ["test2.txt$test3.txt@text/plan"];
var fileNativeData = [];
function GetSelectedFiles(fileData) {
    fileNativeData.push(fileData);
    //fileNativeData = fileData;
    //alert("position:3|" + fileNativeData[fileNativeData.length - 1]);
}

function getNativeDataByName(fileNativeData, relationalName) {
    var nativeData = "";
    if (fileNativeData != undefined) {
        if (fileNativeData.length > 0) {
            angular.forEach(fileNativeData, function (data) {
                if (data.indexOf(relationalName + "$") >= 0) {
                    nativeData = data;
                    return false;
                }
            });
