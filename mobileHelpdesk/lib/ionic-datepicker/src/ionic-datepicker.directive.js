	...
	function IonicDatepicker($ionicPopup, $ionicModal, IonicDatepickerService) {
		return {
			...
			link: function(scope, element, attrs) {
				...
				var inputObject = {
					titleLabel: scope.inputObj.titleLabel || '选择日期',
					closeLabel: scope.inputObj.closeLabel || '关闭',
					todayLabel: scope.inputObj.todayLabel || '今天',
					setLabel: scope.inputObj.setLabel || '选择',
					...
					modalHeaderColor: scope.inputObj.modalHeaderColor || '',
					modalFooterColor: scope.inputObj.modalFooterColor || '',
					...
					dateFormat: scope.inputObj.dateFormat || 'yyyy-MM-dd',
				...
				if (scope.inputObj.templateType.toLowerCase() === 'modal') {
					$ionicModal.fromTemplateUrl('lib/ionic-datepicker/src/ionic-datepicker-modal.html?v=1.1', {
						scope: scope,
				...
				scope.dateClickedAutoClose = function(date) {
					if (!date || Object.keys(date).length === 0) return;
					scope.selctedDateString = date.dateString;
					scope.selctedDateStringCopy = angular.copy(scope.selctedDateString);
					scope.dateSelection.selected = true;
